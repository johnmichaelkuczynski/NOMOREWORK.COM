...
import { FreemiumPaywall } from "./paywall/freemium";
...
Respond with the refined solution only:`;
  ...
  // CRITICAL: Uses IDENTICAL paywall logic as non-streaming endpoint via FreemiumPaywall
  app.post("/api/process-text/stream", async (req, res) => {
    ...
    try {
      ...
      // Set up chunked streaming response with paywall headers
      ...
      // FAIL-CLOSED: Get user safely, default to anonymous if any error
      try {
        if (userId) {
          user = await authService.getUserById(userId);
          if (!user) {
            console.log(`STREAM PAYWALL: User ${userId} not found - treating as anonymous`);
            userId = undefined;
          }
        }
      } catch (error) {
        console.error("STREAM PAYWALL: Error fetching user - treating as anonymous:", error);
        userId = undefined;
        user = null;
      }
      ...
      // FAIL-CLOSED: Get streaming function safely
      ...
      try {
        ...
        res.write(`\n[STREAM_DONE]{"error":"${errorMsg}"}`);
        return res.end();
      }

      // STEP 1: ALWAYS generate the complete solution server-side
      try {
        ...
        console.error("STREAM: Content generation error:", error);
        res.write(`\n[STREAM_DONE]{"error":"${errorMsg}"}`);
        return res.end();
      }
      
      // FAIL-CLOSED: If no content generated, return error
      if (!fullContent.trim()) {
        res.write(`\n[STREAM_DONE]{"error":"No content generated"}`);
        return res.end();
      }

      // STEP 2: Use centralized paywall logic to determine access level
      const paywallResponse = FreemiumPaywall.processContent({
        userId: userId?.toString(),
        user,
        fullContent,
        estimatedTokens: estimatedOutputTokens,
        endpoint: "/api/process-text/stream"
      });

      // STEP 3: Set paywall headers
      Object.entries(paywallResponse.headers).forEach(([key, value]) => {
        res.setHeader(key, value);
      });

      // STEP 4: Stream content progressively with paywall enforcement
      const contentToStream = FreemiumPaywall.getUserContent(paywallResponse);
      const maxBytes = contentToStream.length; // Enforce the paywall limit
      let streamedBytes = 0;
      
      // PROGRESSIVE STREAMING with paywall enforcement
      const words = contentToStream.split(' ');
      for (const word of words) {
        const chunk = (streamedBytes === 0 ? word : ' ' + word);
        
        // FAIL-CLOSED: Stop streaming if we hit the paywall limit
        if (streamedBytes + chunk.length > maxBytes) {
          break;
        }
        ...
        // Simulate real-time streaming delay (can be removed for faster responses)
        await new Promise(resolve => setTimeout(resolve, 10));
      }
      
      // AUDIT LOG: Record the streaming activity
      console.log(`PAYWALL AUDIT: userId=${userId || 'anonymous'}, endpoint=/api/process-text/stream, isPreview=${paywallResponse.isPreview}, percent=${paywallResponse.previewPercent}, originalBytes=${fullContent.length}, sentBytes=${streamedBytes}, assignmentId=pending`);

      // STEP 5: Process graphs (only for full access users)
      let graphData: any[] = [];
      ...
      // STEP 6: Store in database (content based on paywall decision)
      const storageContent = FreemiumPaywall.getStorageContent(paywallResponse);
      const cleanedResponse = storageContent.replace(/GRAPH_DATA_START[\s\S]*?GRAPH_DATA_END/g, "").trim();
      const processingTime = Date.now() - startTime;
      const outputTokens = countTokens(cleanedResponse);
      const wordCount = cleanedResponse.split(/\s+/).length;
      const actualCreditCost = calculateCreditCost(llmProvider, wordCount);
      ...
      try {
        ...
        // STEP 7: Deduct credits only for full access users
        if (userId && !paywallResponse.isPreview) {
          const currentUser = await authService.getUserById(userId);
          if (currentUser && currentUser.username !== "jmkuczynski" && currentUser.username !== "randyjohnson") {
            const newBalance = Math.max(0, (currentUser.tokenBalance || 0) - actualCreditCost);
            await authService.updateUserTokenBalance(userId, newBalance);
          }
        }

        // UPDATE AUDIT LOG with assignment ID
        console.log(`PAYWALL AUDIT FINAL: userId=${userId || 'anonymous'}, endpoint=/api/process-text/stream, isPreview=${paywallResponse.isPreview}, percent=${paywallResponse.previewPercent}, originalBytes=${fullContent.length}, sentBytes=${streamedBytes}, assignmentId=${assignment.id}, creditCost=${actualCreditCost}, stored=${cleanedResponse.length}chars`);

        // STEP 8: Send completion metadata with paywall info
        res.write(`\n[STREAM_DONE]${JSON.stringify({
          partial: paywallResponse.isPreview,
          error: null,
          ...
          processingTime,
          success: true,
          previewPercent: paywallResponse.previewPercent,
          isPreview: paywallResponse.isPreview,
          streamedBytes: streamedBytes,
          storedBytes: cleanedResponse.length
        })}`);

      } catch (dbError) {
        console.error("Database error during streaming:", dbError);
        // FAIL-CLOSED: Database error defaults to preview response
        res.write(`\n[STREAM_DONE]{"error":"Failed to save assignment","partial":true,"isPreview":true}`);
      }
      ...
      // FAIL-CLOSED: Any unexpected error defaults to preview-only, never full content
      try {
        res.write(`\n[STREAM_DONE]{"error":"Streaming failed","partial":true,"isPreview":true}`);
        res.end();
      } catch (writeError) {
        // If we can't even write the error response, fall back to HTTP error
        res.status(500).json({ 
